...
import {
  ...
  queryAiPaintingTaskUsingGet, uploadPictureByUrlUsingPost,
...
// 打开弹窗
const openModal = () => {
  console.log('openModal called') // 添加日志用于调试
  nextTick(() => {
    visible.value = true
  })
}
...
// 关闭弹窗
const closeModal = () => {
  console.log('closeModal called') // 添加日志用于调试
  visible.value = false
  // 清理状态
...
const startPolling = () => {
  ...
  pollingTimer = setInterval(async () => {
    try {
      const res = await queryAiPaintingTaskUsingGet({
        taskId: taskId.value,
      })
      if (res.data.code === 0 && res.data.data) {
...
// 添加调试日志
console.log('ImageOutPainting component mounted, visible:', visible.value)
</script>
